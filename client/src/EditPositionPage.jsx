...
function EditPositionPage() {
  ...
  function handleSubmit(event) {
    ...
    fetchWithAuth(import.meta.env.VITE_API_URL + `/api/positions/${id}`, {
      ...
      body: JSON.stringify({ title, shortDescription, isPublic, maxProjects, projectTags, attributeIds: selectedAttributeIds, version, accessRules })
    })
      .then(data => {
        if (data.message) {
  ...
  return (
     <div className={pageContainer}>
      ...
      <div className="flex flex-col gap-1">
        ...
        <TagsInput className={inputStyle} value={projectTags} onChange={setProjectTags} name="tags" placeHolder="Type a tag and press Enter"/>
      </div>

     <div className="flex flex-col gap-1">
        <label className={labelStyle}>Attributes</label>
        {attributes.map(attribute => (
            <label key={attribute.id} className={`${labelStyle} flex items-center gap-2`}>
            <input
